 ...
 *
 * **NOTE:** It is best practice to check if opening hours are null before passing in a `GMSPlace`
 * (with opening hours already requested) to `GMSPlaceIsOpenRequest`. In some cases, a place may not
 * have any opening hours data available. If a place request with the opening hours property has
 * already been made, and opening hours are null, calling this method using the place response
 * object will result in another billable event.
 *
